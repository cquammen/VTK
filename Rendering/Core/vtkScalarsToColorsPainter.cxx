  ...
  int multiply_with_alpha)
{
  ...
  if (this->ColorTextureMap == 0 ||
    ...
    numberOfColors += 2; // For out-of-range colors
    ...
    for (int i = 0; i < numberOfColors; ++i)
      {
      *scalarTablePtr = range[0] + i * k - k; // minus k for below range color
      if (use_log_scale)
        {
                               ...
                               float &tex_coord_t)     // 2nd tex coord
{
  if (vtkMath::IsNan(scalar_value))
    ...
    // above or below 1000, so we give a warning.  I don't have a
    // better solution.
    if (tex_coord_s > 1000.0f)
      {
      vtkGenericWarningMacro(<< "Texture coordinate clamped to 1000.0. Colors may not appear correct");
      tex_coord_s = 1000.0f;
      }
