...
#
#   Custom test names can be specified by preceding the file name with the
#   custom test name followed by a comma, e.g.
#
#   Test1,Test.cxx
#
#   For tests specified in this format, this function sets the variable
#   'test_name' to 'Test1' and 'test_file' to 'Test.cxx'. For tests specified
#   without a custom name, .e.g
#
#   Test.cxx
#
#   both variables 'test_name' and 'test_file' will be set to the variable
#   'name' defined in the caller.
function(_vtk_test_parse_name)
  ...
  foreach(name IN LISTS names)
    ...
    if(local_TESTING_DATA)
      ...
      if(local_CUSTOM_BASELINES)
        ...
        set(_V -V "DATA{${baseline_dir}/${vtk_test_prefix}${test_name}.png,:}")
      endif()
    endif()
...
function(vtk_add_test_python)
  ...
  foreach(name IN LISTS names)
    ...
    if(NOT local_NO_VALID)
      if(local_NO_RT)
        set(_B -B "DATA{${baseline_dir}/,REGEX:${vtk_test_prefix}${test_name}(_[0-9]+)?.png}")
      else()
        set(_V -V "DATA{${baseline_dir}/${vtk_test_prefix}${test_name}.png,:}")
